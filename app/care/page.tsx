...
const dims: Dim[] = [
  ...
  { key: "analyse", title: "Analyse", question: "Are we asking students to break apart complexity, or just recall and reproduce?", examples: ["Requires decomposing a case, text, dataset, or phenomenon", "Demands comparison across competing frameworks", "Cannot be satisfied by a well-written summary"] },
  { key: "reflect", title: "Reflect", question: "Are we asking students to examine their own assumptions and growth?", examples: ["Student must name a prior belief and how their thinking shifted", "Positions the student inside the argument", "Invites acknowledgement of uncertainty"] },
  { key: "evaluate", title: "Evaluate", question: "Are we asking students to make reasoned judgements, with evidence, in context?", examples: ["Student must choose between options and defend the choice", "Weighs trade-offs", "Contextualises judgement in specific Caribbean realities"] },
];
...
export default function CAREPage() {
  ...
  const aiPrompt = useMemo(() => {
    const list = dims.map(d => "- " + d.title + ": " + (answers[d.key] ?? 0) + "/4").join("\n");
    let p = "A Caribbean university faculty member has used the CARE framework (Consider, Analyse, Reflect, Evaluate) to diagnose how AI-replaceable an existing assessment is. Higher = more irreducibly human.\n\n";
    if (title) p += "Assessment: " + title + "\n\n";
    p += "Scores:\n" + list + "\nOverall AI-resistance: " + Math.round(score*100) + "%\n\n";
    p += "In 3 short paragraphs: (1) what these scores reveal about how a large language model could complete this task, (2) the SINGLE most-impactful redesign move targeting their weakest dimension (concrete and Caribbean-anchored), (3) a sample reworded prompt sentence they could try this semester.";
    return p;
  }, [answers, title, score]);
      ...
      {answered > 0 && (
        <section className="mt-10">
          ...
          <ResultBand score={score} bands={[
            ...
            { min: 85, tone: "sea", label: "Irreducibly human", message: "This measures what students carry. Document and share it." },
          ]} />
          {complete && weakest && (
          ...
          {complete && (
            <AiFeedback systemPrompt="You are a Caribbean assessment design expert and university teacher. You give faculty short, concrete advice rooted in Caribbean realities (CXC traditions, regional examples, multilingual student populations)." userPrompt={aiPrompt} accent="bg-palm" buttonLabel="Generate CARE redesign with AI" helperText="Claude will read your CARE scores and suggest the highest-impact redesign move." />
          )}
